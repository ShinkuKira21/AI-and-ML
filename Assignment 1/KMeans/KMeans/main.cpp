...
int main(int argc, char** argv)
{
    ...
    std::ranges::for_each(cluster.begin(), cluster.end(), [] (MTools::Vector2D<size_t> obj) -> void { printf("x: %li - y: %li\n", obj.x, obj.y); });
    ...
    std::ranges::for_each(cogCluster.begin(), cogCluster.end(), [] (MTools::Vector2D<float> obj) -> void { printf("x: %.2f - y: %.2f\n", obj.x, obj.y); });

    // Tested the VectorDistance function - Works
    //std::cout << MTools::VectorDistance(MTools::Vector2D<float>({2.5, 6.0}), {9, 5}) << std::endl;
    CTools::KMeans::KMeans(&dataCluster, &cogCluster);
